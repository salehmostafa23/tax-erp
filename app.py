...
def _gpc_api_get(endpoint,params=None):
    try:
        r=http_requests.get(f"{GPC_API_BASE}{endpoint}",params=params,headers=_GPC_HEADERS,timeout=30)
        if r.status_code==200:
            data=r.json()
...
def _gpc_fetch_all():
    ...
    if not bricks: return None,"فشل تحميل بيانات Brick"
...
def _save_supplier_codes(data):
    ...
    _sup_search_result.clear()

@st.cache_data(ttl=900, show_spinner=False)
def _sup_search_result(sig, query):
    ...
    if query:
        ...
        filtered=sup_db
    df=pd.DataFrame([{'الكود':p.get('code',''),'الاسم':p.get('name',''),'الوصف':p.get('description',''),'الرقم الضريبي':p.get('tax_number',''),'المورد':p.get('supplier_name','')} for p in filtered])
    buf=BytesIO()
...
def _standalone_supplier_codes_tab():
    ...
    if sup_db:
        ...
        sup_df,sup_xlsx=_sup_search_result(len(sup_db),sup_search.strip().lower())
        if not sup_df.empty:
            if len(sup_db)>1000:
            ...
            st.download_button("📊 تحميل القائمة",data=sup_xlsx,file_name="supplier_codes.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",key="dl_sup")
            if is_admin and st.button("🔄 إعادة تحميل البيانات من GitHub",key="sup_refetch"):
                _load_supplier_codes.clear();_sup_search_result.clear();st.rerun()
...
# ====================== HOME ======================
if page == "🏠 الرئيسية":
    st.markdown('<style>a[href*="manage"],a[data-testid="stManageApp"],section[data-testid="stSidebar"]+div>a,a[title="Manage app"],div[data-testid="stStatusWidget"]{display:none!important;} div[data-testid="stDecoration"]{display:none!important;}</style>',unsafe_allow_html=True)
...
<div class="erp-topbar-right"><a href="https://invoicing.eta.gov.eg/" target="_blank" style="background:linear-gradient(135deg,rgba(0,206,201,.18),rgba(108,92,231,.12));border:1px solid rgba(0,206,201,.35);border-radius:12px;padding:.5rem 1.2rem;color:#00cec9;font-size:.82rem;font-weight:700;text-decoration:none;cursor:pointer;transition:all .3s;display:inline-flex;align-items:center;gap:.5rem;">🔗 فتح بوابة الفواتير الإلكترونية</a></div></div>""", unsafe_allow_html=True)
    ...
    with _tab5:
        ...
        if gpc_cache:
            ...
            st.markdown(f'<div class="erp-section" style="margin-top:1rem"><div class="erp-section-dot" style="background:#fdcb6e;"></div><h3>جميع Bricks ({len(bricks_list)})</h3></div>',unsafe_allow_html=True)
            if bricks_list:
                ...
                excel_buf_gpc.seek(0)
                st.download_button("📊 تحميل جميع Bricks",data=excel_buf_gpc.getvalue(),file_name="gpc_bricks.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",key="dl_gpc")
            ...
            st.markdown("""<div class="erp-empty" style="padding:3rem;margin-top:1rem;text-align:center;">
                ...
                <p style="color:var(--text2);">اضضغط تحديث لتحميل تصنيف Brick من GS1</p>
                <p style="color:var(--text2);font-size:.78rem;margin-top:.5rem;">Segment → Family → Class — يدعم البحث بالعربي والإنجليزي</p>
            </div>""",unsafe_allow_html=True)
...
<div class="erp-topbar-right"><span class="erp-badge">🏷️ أكواد</span></div></div>""", unsafe_allow_html=True)
    ...
    _codes_labels=["🔗 أكواد البورتال","🔬 GPC Browser","🏷️ أكواد GS1","🏷️ أكواد الموردين"]
    if hasattr(st,"pills"):
        _codes_sel=st.pills("القسم",_codes_labels,selection_mode="single",default="🔗 أكواد البورتال",key="codes_sec_pills",label_visibility="collapsed")
    else:
        _codes_sel=st.radio("القسم",_codes_labels,horizontal=True,key="codes_sec_radio")
    if not _codes_sel: _codes_sel="🔗 أكواد البورتال"
    if _codes_sel=="🔬 GPC Browser": _standalone_gpc_tab()
    elif _codes_sel=="🏷️ أكواد GS1": _standalone_gs1_tab()
    elif _codes_sel=="🏷️ أكواد الموردين": _standalone_supplier_codes_tab()
    else: _standalone_portal_tab()
...
<div class="erp-topbar-right">{_portal_led()}</div></div>""", unsafe_allow_html=True)
    ...
    _tab_ws,_tab_rent_grp=st.tabs(["📦 مبيعات الجملة","🏢 فواتير الإيجارات"])

    with _tab_ws:
        st.info("تاب مبيعات الجملة قيد التطوير - جارٍ التحضير")

    with _tab_rent_grp:
        _tab_rent,_tab_xl=st.tabs(["✍️ التوقيع مباشر","📊 تحويل الفاتورة لأكسيل"])
        with _tab_rent:
